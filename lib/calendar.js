...
export default function Calendar(cal) {
	...
	this.getDayType = date => {
		// check if date is april 15th 2022
		if (date.getDate() == 15 && date.getMonth() == 3) {
			return SPECIAL_DAY;
		}
	...
	this.isEndOfDay = (period, dayType) => {
		if (dayType == COMMON_DAY) {
			return period > 11;
		}
		return period > 10;
	};

	this.getSchedule = dayType => {
		if (!(dayType in SCHEDULES)) return [];
